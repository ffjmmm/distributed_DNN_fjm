...
class lossy_Conv2d_new(nn.Module):
    ...
    def forward(self, x):
        ...
        def split(x, pieces):
            ...
            '''
            x11 = torch.empty((dim[0], dim[1], dim[2] // pieces[0], dim[3] // pieces[1]))
            x12 = torch.empty((dim[0], dim[1], dim[2] // pieces[0], dim[3] // pieces[1]))
            x21 = torch.empty((dim[0], dim[1], dim[2] // pieces[0], dim[3] // pieces[1]))
            x22 = torch.empty((dim[0], dim[1], dim[2] // pieces[0], dim[3] // pieces[1]))
            '''
            x11 = x11.cuda()
            x12 = x12.cuda()
            x21 = x21.cuda()
            ...
            '''
            x11.copy_(x[:, :, 0: dim[2] // pieces[0], 0: dim[3] // pieces[1]])
            x12.copy_(x[:, :, 0: dim[2] // pieces[0], dim[3] // pieces[1]: dim[3]])
            x21.copy_(x[:, :, dim[2] // pieces[0]: dim[2], 0: dim[3] // pieces[1]])
            ...
            x12 = x[:, :, 0: dim[2] // pieces[0] + 1, dim[3] - 1 // pieces[1]: dim[3]]
            x21 = x[:, :, dim[2] - 1 // pieces[0]: dim[2], 0: dim[3] // pieces[1] + 1]
            x22 = x[:, :, dim[2] - 1 // pieces[0]: dim[2], dim[3] - 1 // pieces[1]: dim[3]]
            '''
            ...
            '''
            x11 = F.dropout(x11, p=0.5, training=True) * 0.5
            x12 = F.dropout(x12, p=0.5, training=True) * 0.5
            x21 = F.dropout(x21, p=0.5, training=True) * 0.5
            x22 = F.dropout(x22, p=0.5, training=True) * 0.5
